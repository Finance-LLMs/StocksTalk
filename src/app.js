...
function updateSpeakingStatus(mode) {
    ...
    if (isSpeaking) {
        ...
        if (sqlQueryRequested) {
            console.log('Agent finished speaking after SQL query request');
            // Since we can't directly get the response text from the conversation object,
            // we'll use a workaround where the user manually provides the SQL query
            // or we implement a different approach to capture the response
            
            // For now, let's prompt the user to copy-paste the SQL query
            setTimeout(() => {
                const userResponse = prompt('Please paste the SQL query generated by the AI:');
                if (userResponse && userResponse.trim()) {
                    processSQLResponse(userResponse.trim());
                } else {
                    // Reset the button if user cancels
                    sqlQueryRequested = false;
                    if (sqlQueryButton) {
                        sqlQueryButton.disabled = false;
                        sqlQueryButton.classList.remove('loading');
                        sqlQueryButton.innerHTML = `
...
async function generateSQLQuery() {
    if (conversation && !sqlQueryRequested) {
        try {
            // Set flag to indicate we're expecting a SQL query
...
async function processSQLResponse(responseText) {
    try {
        ...
        sqlQuery = sqlQuery.replace(/^(Here's the SQL query|The SQL query is|SQL query:)\\s*/i, '');
        sqlQuery = sqlQuery.replace(/```sql\\s*/, '').replace(/```\\s*$/, '');
        sqlQuery = sqlQuery.trim();
        
        if (!sqlQuery) {
            throw new Error('No SQL query found in AI response');
        }
